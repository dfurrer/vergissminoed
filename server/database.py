...
class Database(object):
  ...
  def load(self, data_dir):
    ...
    for pos_filename in pos_filenames:
      if not self.re_json.match(pos_filename):
        continue
      ...
      with open(pos_path, 'r') as f:
        ...
        try:
          ...
          print 'File %s could not be json parsed.' % repr(pos_path)
          raise
    ...
    for prodarticles_filename in prodarticles_filenames:
      if not self.re_json.match(prodarticles_filename):
        continue
    ...
    for products_filename in products_filenames:
      if not self.re_json.match(products_filename):
        continue
  ...
  def compute_customer_model(self, customerid):
    ...
    for purchase in self.pos[customerid]:
      ...
      quant = float(purchase['quantNorm'])
      purchase_date = purchase['date']

      if quant <= 0.0:
        raise RuntimeError(
          'Purchase is invalid for customer %s, since quantNorm is less-equal 0.0: %s' % (
          repr(customerid), repr(purchase)))
    ...
    for (productid, product_model) in product_models.iteritems():
      if self.count_threshold < 1:
        raise RuntimeError("count_threshold must be at least 1, now it's %s." % 
                          repr(self.count_threshold))

      if product_model['count'] <= self.count_threshold  or \
    ...
    for (productid, product_model) in product_models.iteritems():
      if product_model['last_stock'] <= 0.0:
        raise RuntimeError(
          "last_stock must be greater than zero, but it's not for product model: %s" % repr(product_model))
  ...
  def _parse_purchase_dates(self, customerid):
    ...
    def cmp_on_date(me,other):
      return (me['date'] - other['date']).days

    self.pos[customerid].sort(cmp_on_date)
